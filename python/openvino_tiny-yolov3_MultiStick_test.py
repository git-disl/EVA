...
try:
    from armv7l.openvino.inference_engine import IECore
except:
...
def camThread(LABELS, results, frameBuffer, camera_width, camera_height, vidfps, vidfile, number_of_ncs):
    global fps
    global detectfps
    global lastresults
    global framecount
    ...
    while True:
        ...
        if not s:
            if cv2.waitKey(wait_key_time) & 0xFF == ord('q'):
                # save data
                pickle.dump((detectionTimes, detectionFPSs), open("tiny-yolov3-" + str(number_of_ncs) + ".p", "wb"))
                sys.exit(0)
            continue
        if frameBuffer.full():
            frameBuffer.get()
        ...
        if not results.empty():
            ...
            for obj in objects:
                ...
                if confidence > 0.2:
                    ...
                    obj.xmax = min(obj.xmax, color_image.shape[1])
                    obj.ymax = min(obj.ymax, color_image.shape[0])
                    obj.xmin = max(obj.xmin, 0)
                    obj.ymin = max(obj.ymin, 0)
                    #print(color_image.shape, obj.xmin, obj.ymin, obj.xmax, obj.ymax, box_color, box_thickness)
                    cv2.rectangle(color_image, (int(obj.xmin), int(obj.ymin)), (int(obj.xmax), int(obj.ymax)), box_color, box_thickness)
        ...
        if framecount >= 15:
            ...
            detectionTimes.append(time2)
            detectionFPSs.append(detectframecount/time2)
            framecount = 0
            detectframecount = 0
            time1 = 0
            time2 = 0
...
class NcsWorker(object):
    ...
    def __init__(self, devid, frameBuffer, results, camera_width, camera_height, number_of_ncs, vidfps):
        ...
        self.model_xml = "./pretrained-models/frozen_tiny_yolo_v3.xml"
        self.model_bin = "./pretrained-models/frozen_tiny_yolo_v3.bin"
        ...
        self.inferred_cnt = 0
        self.iecore = IECore()
        self.net = self.iecore.read_network(model=self.model_xml, weights=self.model_bin)
        self.input_blob = next(iter(self.net.input_info))
        self.exec_net = self.iecore.load_network(network=self.net, device_name='MYRIAD', num_requests=self.num_requests)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-vf', '--videofile', dest='video_file', type=str, default="../demo/testvideo3.mp4",help='Video for evaluation')
    args = parser.parse_args()

    number_of_ncs = args.number_of_ncs
    video_file = args.video_file
    from utils import getResolutionAndFPS
    camera_width, camera_height, vidfps = getResolutionAndFPS(video_file)

    try:

        mp.set_start_method('forkserver')
        frameBuffer = mp.Queue(12)
        results = mp.Queue()
        ...
        # Start streaming
        p = mp.Process(target=camThread, args=(LABELS, results, frameBuffer, camera_width, camera_height, vidfps, video_file, number_of_ncs), daemon=True)
        p.start()
        processes.append(p)
