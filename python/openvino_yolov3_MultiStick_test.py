    ...
    while True:
        ...
        if not s:
            if cv2.waitKey(wait_key_time) & 0xFF == ord('q'):
                # save data
                pickle.dump((detectionTimes, detectionFPSs), open("yolov3-" + str(number_of_ncs) + ".p", "wb"))
                sys.exit(0)
            continue
        if frameBuffer.full():
            frameBuffer.get()
        ...
        if cv2.waitKey(wait_key_time)&0xFF == ord('q'):
            # save data
            pickle.dump((detectionTimes, detectionFPSs), open("yolov3-" + str(number_of_ncs) + "-middle.p", "wb"))
            sys.exit(0)
