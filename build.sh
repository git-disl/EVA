...
ODCPP_PATH="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"

printf "\nSetting environment variables for building demos...\n"

if [ -z "$INTEL_OPENVINO_DIR" ]; then
    if [ -e "$ODCPP_PATH/../../bin/setupvars.sh" ]; then
        setvars_path="$ODCPP_PATH/../../bin/setupvars.sh"
    elif [ -e "$ODCPP_PATH/../../../bin/setupvars.sh" ]; then
        setvars_path="$ODCPP_PATH/../../../bin/setupvars.sh"
    else
...
(cd "$build_dir" && cmake -DCMAKE_BUILD_TYPE=Release "${extra_cmake_opts[@]}" "$ODCPP_PATH")
cmake --build "$build_dir" -- "$NUM_THREADS"

printf "\nBuild completed, you can find binaries for all binaries in the %s subfolder.\n\n" "$build_dir/$OS_PATH/Release"
