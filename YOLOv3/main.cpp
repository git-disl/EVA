...
void ParseYOLOV3Output(const YoloParams &params, const std::string & output_name,
                       ...
                       std::vector<DetectionObject> &allObjects) {
    ...
    for (int i = 0; i < side_square; ++i) {
        ...
        for (int n = 0; n < params.num; ++n) {
            ...
            for (int j = 0; j < params.classes; ++j) {
                ...
                if (scale >= threshold && prob >= threshold) {
...
int main(int argc, char *argv[]) {
    try {
        ...
        outfile.open(FLAGS_outfilepath);
        ...
        do {
            ...
            } else if (!isSkipped && !isModeChanged) {
                FrameToBlob(frame, async_infer_request_curr, inputName);
            }
            ...
            if (n_skipped_frames < FLAGS_nskip || (FLAGS_nskip == -1 && OK != async_infer_request_curr->Wait(IInferRequest::WaitMode::STATUS_ONLY))) {
                isSkipped = true;
                n_skipped_frames += 1;
                ...
                isSkipped = false;
                std::cout << "Skipped " << n_skipped_frames << std::endl;
                n_skipped_frames = 0;
                if (FLAGS_nskip >= 0 && OK != async_infer_request_curr->Wait(IInferRequest::WaitMode::RESULT_READY)) {
                    throw std::runtime_error("Waiting for inference results error");
                }
                ...
                //cv::putText(frame, out.str(), cv::Point2f(0, 20), cv::FONT_HERSHEY_TRIPLEX, 0.6, cv::Scalar(0, 0, 255));
                // if (!isAsyncMode) {  // In the true async mode, there is no way to measure detection time directly
                //     out.str("");
                //     out << "Detection time  : " << std::fixed << std::setprecision(1) << detection.count()
                //         << " ms ("
                //         << 1000.0 / detection.count() << " fps)";
                //     cv::putText(frame, out.str(), cv::Point2f(0, 25), cv::FONT_HERSHEY_TRIPLEX, 0.6,
                //                 cv::Scalar(255, 0, 0));
                // }
                ...
                for (auto &output : outputInfo) {
                    ...
                    ParseYOLOV3Output(yoloParams[output_name], output_name, blob, resized_im_h, resized_im_w, frame.rows, frame.cols, FLAGS_t, objects, allObjects);
                }
            ...
            if (!FLAGS_no_show) {
                ...
                if(FLAGS_save_image_path.length() > 0) {
                        cv::imwrite(FLAGS_save_image_path + std::to_string(frameId) + ".png", frame);
                }
            }
            ...
            for (auto &d : allObjects) {
                    //std::cout << imgIdx << "," << d.xmin << "," << d.ymin << "," << d.xmax << "," << d.ymax << "," << d.confidence << "," << d.class_id << std::endl;
                    outfile << frameId << "," << d.xmin << "," << d.ymin << "," << d.xmax << "," << d.ymax << "," << d.confidence << "," << d.class_id << std::endl;
            }
            frameId ++;
            ...
            int key;
            if (FLAGS_nskip == 0 || !isSkipped) {
                key = cv::waitKey(1);
            } else {
                key = cv::waitKey(1000.0 / capFPS);
            }
