...
// thread-safe queue modified from https://codetrips.com/2020/07/26/modern-c-writing-a-thread-safe-queue/
template<typename T>
...
class NcsClassifier{
	public:
		...
		int next_request_id;
		ThreadsafeQueue<std::pair<int,Mat> > * queue;
		...
		NcsClassifier(int id, ThreadsafeQueue<std::pair<int, Mat> > * queue, std::string model_xml);
...
NcsClassifier::NcsClassifier(int id, ThreadsafeQueue<std::pair<int, Mat> > * queue, std::string model_xml){
	    this->id = id;
...
void inference_job_async(ThreadsafeQueue<std::pair<int, Mat> > * job_queue, NcsClassifier ncs_classifer, std::atomic<bool> * isFrameFinished){
    std::cout << "Starting Inference, Job Queue: " << job_queue -> size() << std::endl;
    int xidx = 0;
    Mat xfile;
    while (true){
        auto elem = job_queue->pop();
        if(elem) {
            xidx = elem->first;
            xfile = elem->second;
...
void video_input_stream_async(std::list <std::string> xfilelist, int num_devices, ThreadsafeQueue<std::pair<int, Mat> > ** job_queue_arr, std::atomic<bool> * isFrameFinished){
    std::cout << "Starting Reading Video Stream" << std::endl;
    int i = 0;
    for (std::string mfile : xfilelist){
        Mat image = imread(mfile);
        job_queue_arr[i%num_devices]->push({i, image});
        i++;
    }
    // One extra images;
    //ncs_classifer.predict_async(xidx+1, xfile);
    (*isFrameFinished) = true;
    std::cout << "Finish reading all frames." << std::endl;
}

class Scheduler{
	public:
	    ThreadsafeQueue<std::pair<int, Mat> > ** queueArr;
        std::atomic<bool> * isFrameFinished;
...
Scheduler::Scheduler(int deviceids[], int size, std::string model_xml){
    this -> isFrameFinished = new std::atomic<bool>(false);
    this -> queueArr = new  ThreadsafeQueue<std::pair<int, Mat> > * [size];
    this -> num_devices = size;
    init_workers(deviceids, size, model_xml);
}

void Scheduler::init_workers(int ids[], int size, std::string model_xml){
    for (int id=0; id<size; id++){
        this -> queueArr[id] =  new ThreadsafeQueue<std::pair<int, Mat> >();
        std::cout << "creating NcsClassifier for Device " << id << std::endl;
        this->workers.push_back(NcsClassifier(ids[id], this->queueArr[id], model_xml));
    }
}
...
void Scheduler::start(std::list <std::string> xfilelist, std::string outfilepath){
    ...
    threads.push_back(std::thread(video_input_stream_async, xfilelist, this->num_devices, this->queueArr, this->isFrameFinished));

    std::chrono::high_resolution_clock::time_point start_time = std::chrono::high_resolution_clock::now();
    // schedule workers
    int id = 0;
    for (auto const& worker : this->workers){
	    threads.push_back(std::thread(inference_job_async, this->queueArr[id], worker, this->isFrameFinished));
        id++;
    }
    ...
    for (auto const& worker : this->workers) {
        std::cout << "Worker ID: " << worker.id << " ; Number of images processed: " << *worker.num_frame_processed << std::endl;
