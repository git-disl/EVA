...
void video_input_stream_async(std::list <std::string> xfilelist, ThreadsafeQueue<std::pair<int, Mat> > * job_queue, std::atomic<bool> * isFrameFinished){
    std::cout << "Starting Reading Video Stream, Job Queue: " << job_queue->size() << std::endl;
    int i = 0;
    for (std::string mfile : xfilelist){
        Mat image = imread(mfile);
        job_queue->push({i, image});
        i++;
    }
    // One extra images;
    //ncs_classifer.predict_async(xidx+1, xfile);
...
Scheduler::Scheduler(int deviceids[], int size, std::string model_xml){
    ...
    this -> queue = new  ThreadsafeQueue<std::pair<int, Mat> >();
...
void Scheduler::start(std::list <std::string> xfilelist, std::string outfilepath){
    ...
    threads.push_back(std::thread(video_input_stream_async, xfilelist, this->queue, this->isFrameFinished));

    std::chrono::high_resolution_clock::time_point start_time = std::chrono::high_resolution_clock::now();
    // schedule workers
    for (auto const& worker : this->workers){
	    threads.push_back(std::thread(inference_job_async, this->queue, worker, this->isFrameFinished));
    }
