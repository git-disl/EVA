...
class DetectionObject{
    public:
        ...
        float confidence;
        int label;
        DetectionObject(int _xmin, int _ymin, int _xmax, int _ymax, float _conf, int _label) {
            this->xmin = _xmin;
            this->ymin = _ymin;
            this->xmax = _xmax;
            this->ymax = _ymax;
        ...
        while (true) {
            ...
            if (requestResult.output) {
                ...
                for (size_t i = 0; i < maxProposalCount; i++) {
                    ...
                    allObjects.push_back({(int) xmin, (int) ymin, (int) xmax, (int) ymax, confidence, (int) label});
                ...
                if (!FLAGS_no_show) {
                    ...
                    if(FLAGS_save_image_path.length() > 0) {
                        cv::imwrite(FLAGS_save_image_path + std::to_string(frameIdx) + ".png", requestResult.frame);
                    }
                ...
                for (auto &d : allObjects) {
                    ...
                    outfile << frameIdx << "," << d.xmin << "," << d.ymin << "," << d.xmax << "," << d.ymax << "," << d.confidence << "," << d.label << std::endl;
                }
                frameIdx ++;
                allObjects.clear();
